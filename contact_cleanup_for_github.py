...
df = df.merge(account_df, how='left' , on= ['AccountId'])
...
#dup_df[dup_df['Name'] == fullname_duplicates_list[2]]
...
#email_groups = email_groups.sort_values('Count', ascending = False)
...
for val in email_group_duplicates_list:
    ...
    for i in range(len(id_lst)):
        ...
        dup_df.loc[dup_df.Id == id_lst[i], 'DuplicateIds'] = id_str
#email_groups.sort_values('Count', ascending = False)
...
print('Writing resulting df to contacts_with_scores.csv, this csv has partial and exact duplicates')
dup_df.to_csv('contacts_with_scores.csv', encoding='utf-8')
...
#populate columns for parent contact id and sub contact ids
def return_newest_id(ex):
    d = {}
    newest_date = [3000,0,0]
    newest_id = ''
...
exact_dup_df['DuplicateIds'] = exact_dup_df['IdsTuple'].apply(lambda x : x[1])
...
print('writing final csv: exact_contact_duplicates.csv')
